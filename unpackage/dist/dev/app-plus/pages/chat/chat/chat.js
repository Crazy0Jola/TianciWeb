...
var dom = weex.requireModule('dom');var _this;var JIM = getApp().globalData.JIM;var _default = { components: { freeNavBar: _freeNavBar.default, freeIconButton: _freeIconButton.default, freeChatItem: _freeChatItem.default, freePopup: _freePopup.default, freeMainButton: _freeMainButton.default }, data: function data() {return { // 模式 text输入文字，emoticon表情，action操作，audio音频
      mode: "text", username: "", nickname: "", avator: "/static/images/userpic.png", myUsername: "", myAvator: "/static/images/userpic.png", // 扩展菜单列表
      ...
      KeyboardHeight: 0, menus: [{ name: "复制", event: '' }, { name: "发送给朋友", event: '' }, { name: "收藏", event: '' }, { name: "删除", event: '' }, { name: "多选", event: '' }, { name: "撤回", event: 'removeChatItem' }], navBarHeight: 0, list: [], // {
      ...
      unRecord: false };}, onLoad: function onLoad(e) {_this = this;_this.username = e.username;_this.nickname = e.nickname;_this.myUsername = uni.getStorageSync('setUserData').username;}, onShow: function onShow() {uni.$on("get_msg", function () {var msg = uni.getStorageSync(_this.username);_this.list = msg;var pageToBottomTimer = setTimeout(function () {_this.pageToBottom();clearTimeout(pageToBottomTimer);}, 200);});var msg = uni.getStorageSync(_this.username);JIM.resetUnreadCount({ 'username': _this.username });if (msg) {_this.list = msg;var pageToBottomTimer = setTimeout(function () {_this.pageToBottom();clearTimeout(pageToBottomTimer);
      }, 500);
    } else {
      uni.showLoading({
        "title": "正在获取聊天记录..." });
      ...
      uni.request({
        ...
        success: function success(res) {
          console.log(__f__(res.data, " at pages\\chat\\chat\\chat.nvue:314"));
          ...
          uni.hideLoading();
          uni.setStorageSync(_this.username, msg);
        } });
  ...
  computed: {
    ...
    getImg: function getImg() {
      ...
      JIM.getResource({ 'media_id': this.item.msg_body.media_id }).onSuccess(function (data) {
        _this.url = data.url;
        console.log(__f__('success:' + JSON.stringify(data), " at components\\free-ui\\free-chat-item.vue:136"));
      }).onFail(function (data) {
        console.log(__f__('error:' + JSON.stringify(data), " at components\\free-ui\\free-chat-item.vue:138"));
      });
      return _this.url;
    } },
      ...
      if (newVal) {
        ...
        _this2.$nextTick(function () {
          animation.transition(_this2.$refs.isremove, {
            ...
            timingFunction: 'ease' },
          function () {
            console.log(__f__('动画执行结束', " at components\\free-ui\\free-chat-item.vue:161"));
          });
        });
      }
    });
