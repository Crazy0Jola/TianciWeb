...
/* harmony import */ var _pages_chat_chat_set_chat_set_nvue_mpType_page__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./pages/chat/chat-set/chat-set.nvue?mpType=page */ 162);
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _chat_set_nvue_vue_type_template_id_4ca5b25e_mpType_page__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./chat-set.nvue?vue&type=template&id=4ca5b25e&mpType=page */ 163);
/* harmony import */ var _chat_set_nvue_vue_type_script_lang_js_mpType_page__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./chat-set.nvue?vue&type=script&lang=js&mpType=page */ 165);
...
var _freeDivider = _interopRequireDefault(__webpack_require__(/*! @/components/free-ui/free-divider.vue */ 156));
...
//
//
var JIM = getApp().globalData.JIM;var SERVER_API = getApp().globalData.SERVER_API;var _this;var _default = { components: { freeNavBar: _freeNavBar.default, freeAvater: _freeAvater.default, freeDivider: _freeDivider.default, freeListItem: _freeListItem.default }, data: function data() {return { gid: "", gname: "", gdes: "", //群公告
      memberList: [], //成员列表
      gavatar: '/static/images/grouppic.jpg', //群头像
      isAdmin: false, myUsername: "" };}, computed: { getNoDisturb: function getNoDisturb() {return uni.getStorageSync("noDisturb" + _this.gid) || false;}, getIsTop: function getIsTop() {return uni.getStorageSync(_this.gid + "isTop") || false;}, getDesStyle: function getDesStyle() {return _this.gdes ? '' : 'height:0rpx;';} }, onShow: function onShow() {JIM.getGroupInfo({ 'gid': _this.gid }).onSuccess(function (data) {console.log(__f__(data, " at pages\\chat\\chat-set\\chat-set.nvue:122"));console.log(__f__(JSON.stringify(data), " at pages\\chat\\chat-set\\chat-set.nvue:123"));_this.gdes = data.group_info.desc;_this.gname = data.group_info.name;if (data.group_info.avatar) {JIM.getResource({ 'media_id': data.group_info.avatar }).onSuccess(function (data) {_this.gavatar = data.url;}).onFail(function (data) {uni.showToast({ "title": (0, _ErrorCode.default)(data.code), "position": "bottom" });});}}).onFail(function (data) {uni.showToast({ "title": (0, _ErrorCode.default)(data.code), "position": "bottom" });});JIM.getGroupMembers({ 'gid': _this.gid }).onSuccess(function (data) {var tokenList = [];var len = data.member_list.length;_this.memberList = data.member_list;for (var i = 0; i < len; i++) {if (data.member_list[i].username == _this.myUsername && data.member_list[i].flag == 1) {_this.isAdmin = true;}tokenList.push(data.member_list[i].username);}console.log(__f__(tokenList, " at pages\\chat\\chat-set\\chat-set.nvue:158"));uni.request({ url: SERVER_API + 'appUser/getUserPhotoByToken', data: tokenList, header: { token: uni.getStorageSync("setUserData").token }, method: "POST",
        success: function success(res) {
          console.log(__f__(res, " at pages\\chat\\chat-set\\chat-set.nvue:167"));
          for (var i = 0; i < len; i++) {
            var name = _this.memberList[i].username;
            _this.memberList[i].avatar = res.data.result[name];
          }
          console.log(__f__(JSON.stringify(_this.memberList), " at pages\\chat\\chat-set\\chat-set.nvue:172"));
        } });
  ...
  methods: {
    ...
    addMember: function addMember() {
      console.log(__f__(_this.gname, " at pages\\chat\\chat-set\\chat-set.nvue:279"));
      uni.navigateTo({
    ...
    exitGroup: function exitGroup() {
      uni.showModal({
        title: '提示',
        content: _this.isAdmin ? '确定解散该群聊？' : '确定退出该群聊？',
        success: function success(res) {
          if (res.confirm) {
            if (_this.isAdmin) {
              JIM.dissolveGroup({
